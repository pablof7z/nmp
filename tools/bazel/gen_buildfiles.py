...
# No first-party crates are hand-maintained: every workspace member is
...
def all_features_closure(pkg):
    ...
    Distinct from the workspace-unified build, where a package gets only the
    features some other workspace member happens to request.
...
def gen_for(pkg, prod_feats, prod_opt, test_feats, test_opt_all, bin_label_by_name, root):
    ...
    # A crate declaring staticlib/cdylib crate types gets those emitted with
    # its own `--all-features` closure rather than the workspace-unified set,
    # since a shipped C-ABI artifact is built for its own feature set, not for
    # whatever some other workspace member happens to request. `:lib` continues
    # to model the workspace-unified build.
    #
    # No workspace member declares an extra crate type today -- `nmp-ffi` was
    # the only one and it is deleted -- so this path is currently unexercised.
