...
- **~~DM inbox routing incorrect (M3-D)~~ CLOSED (#19), then removed (#839).** `WriteRouting::ToInboxes` used to fall back to the union of recipients' *write*+extra relays because `RelayDirectory` had no read/inbox accessor. #19 corrected that branch to consume only NIP-65 read-marked relays. #839 later removed the generic recipient route, its persistence arm, and its native projections entirely: an independently supplied recipient list cannot prove that the event schema owns those recipients or that its body agrees with them. Recipient delivery may return only through a protocol-owned operation that binds its complete body and bounded route together. `RelayDirectory::read_relays` remains valid directory truth; the removed generic write path no longer consumes it.
...
- **`nmp-media` provides the STANDALONE staged composition seam (prepare → upload → compose) with separated failure domains, but not the durable upload, the FFI projection, or BUD-03 server-list placement (#559, epic #216 T15-C-MEDIA-COMPOSITION).** The opt-in crate wires the app-facing pipeline `Sha256Hash → signed authorization → VerifiedUpload → kind:20 draft` into three witness-typed stages so a skipped/failed stage is unrepresentable: `prepare` (holds the exact bytes it hashed and authorized — uploading those held bytes makes an authorized-hash/uploaded-bytes mismatch structurally impossible), the async standalone `PreparedUpload::upload` (a used-once obligation yielding a verified `UploadedAsset`), and `compose_picture` (the final unsigned kind:20 the app hands to the EXISTING `publish()` → WriteIntent path). It defines no event schema of its own — composition is not schema ownership (`routing-and-ownership.md` §3.2.1) — and it never signs (signing is upstream) or publishes (relay/publish is downstream). The three failure domains are three SEPARATE types (`PrepareError`, `MediaUploadError`, `MediaComposeError`) so an upload failure can never be pattern-matched or `?`-merged as a compose failure; `MediaUploadError::Blossom` preserves the whole separated Blossom `UploadError` taxonomy intact. Deliberately NOT in this unit: the upload half is NOT crash-durable (the engine-integrated durable-upload obligation — persisted intent / reattachable receipt / HTTP-publish Effect / blob persistence — is the ADDITIVE #562, whose witness types are identical to these); the compose stage has no FFI/Swift/Kotlin projection (batched with the nip68 projection, compile-gated) — the prepare+upload stages now do, through #971's one-shot engine-authorized upload below; and BUD-03 kind:10063 server-list placement is still deferred.

- **The one-shot engine-authorized Blossom upload covers upload, and only upload (#971).** `Engine::upload_blossom` (`crates/nmp/src/blossom_upload.rs`) composes `nmp_media::prepare` + `PreparedUpload::upload` + `nmp_blossom::BlossomClient` into ONE call that owns the whole security-sensitive transaction — active author, engine clock, exact-byte SHA-256, BUD-11 kind:24242 composition, governed signing, re-validation before any HTTP, and the hardened `PUT /upload` — projected through `nmp-ffi` (`NmpEngine::upload_blossom` + `NmpBlossomUploadHandle`), Swift (`NMPEngine.uploadBlossom`) and Kotlin (`NMPEngine.uploadBlossom`). It is deliberately NOT the durable upload #562 rejected and this issue explicitly retained the rejection of: it is process-local and non-durable, and it creates no write intent, receipt, store row, journal entry, outbox lane, retry owner, scheduler or third workload noun. Cancellation after the request has been transmitted is an OBSERVATION GAP, not a rollback — the local operation stops, and NMP never claims whether the remote stored the bytes. It sets no policy either: there is no size cap, no concurrency cap and no aggregate memory ceiling, because how large a blob a user may upload and how many uploads an app runs are the app's decisions (the owner's ruling on PR #1020). **Gaps:** the other verbs (`mirror`/`delete`/`list`) and any Blossom author that is not the active account still use the low-level `Blossom.swift`/`Blossom.kt` choreography, which is why that surface remains; #971's "source gate prevents Swift from regaining the low-level signing choreography for this operation" is NOT built, because wiring a new `scripts/check-*.sh` into CI requires editing `.github/workflows/ci.yml`, a path `scripts/check-surface-governance.sh` fails any ordinary PR for touching; and `PreparedUpload::upload` still hands `nmp_blossom::BlossomClient::upload` a borrowed slice that reqwest copies into its request body, so a blob is briefly resident twice — bounded and harmless when NMP capped uploads at 16 MiB, worth revisiting now that it does not.
