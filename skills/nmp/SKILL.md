...
- Do not expose secret keys in logs, fixtures, screenshots, or source. The bundled `NMPInsecureFileAccountStore` variants are explicitly insecure development conveniences; prefer the shipped platform-vault conformers (`NMPKeychainAccountStore` on Swift, the `java.security.KeyStore`-backed store on Kotlin), which satisfy the same `NMPLocalAccountCheckpoint` contract.
- Do not promise app-controlled retries or typed pending-row metadata: those are not current public capabilities. `RelayWaiting::BackingOff` is evidence from NMP's engine-owned durable scheduler, not an app retry door. Write cancellation before signing, publish-queue enumeration and entry removal, correlation-token reattachment, `maxRelays`/`allowedLocalRelayHosts` config, populated `AuthPhase`, and secure platform-vault account stores all do exist on the native tiers — check the current surface before calling something absent. No tier exposes any application-configurable worker/task/thread capacity: #704 removed it entirely.
