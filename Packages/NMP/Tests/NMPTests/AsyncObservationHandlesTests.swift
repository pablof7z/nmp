...
final class AsyncObservationHandlesTests: XCTestCase {
    ...
    func testHundredsOfSimultaneousObservationsOpenWithNoCapacityCeiling() throws {
        ...
        for k in 0..<250 {
            queries.append(try engine.observe(.single(NMPDemand(selection: NMPFilter(kinds: [UInt16(9_000 + k % 400)]), source: .public))))
        }
        for _ in 0..<40 {
    ...
    func testManySimultaneousIteratingObservationsCancelCleanly() async throws {
        ...
        let queries = try (0..<count).map { k in
            try engine.observe(.single(NMPDemand(selection: NMPFilter(kinds: [UInt16(7_000 + k)]), source: .public)))
        }
    ...
    func testCancelWakesAParkedNextImmediatelyWithNoPostCancelFrameAndIsIdempotent() async throws {
        ...
        let query = try engine.observe(.single(NMPDemand(selection: NMPFilter(kinds: [8_811]), source: .public)))
    ...
    func testNormalLoopBreakWithdrawsDemandAndReleasesIteratorClaim() async throws {
        ...
        let query = try engine.observe(.single(NMPDemand(selection: NMPFilter(kinds: [8_812]), source: .public)))
    ...
    func testTwoIndependentObservationsBothDeliverConcurrentlyWithNoError() async throws {
        ...
        let a = try engine.observe(.single(NMPDemand(selection: NMPFilter(kinds: [8_821]), source: .public)))
        let b = try engine.observe(.single(NMPDemand(selection: NMPFilter(kinds: [8_822]), source: .public)))
    ...
    func testTwoConcurrentIteratorsOverOneNMPQueryAreLoudNeverHung() async throws {
        ...
        let query = try engine.observe(.single(NMPDemand(selection: NMPFilter(kinds: [8_831]), source: .public)))
