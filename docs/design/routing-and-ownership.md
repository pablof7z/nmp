...
resolved-graph property rather than voluntary registration: every `nmp-*`
package is assigned a single role (for example, protocol crates like
`nmp-nip29` are `pure-protocol` by family, not by a per-crate exception), and
each role has a fixed set of roles it may depend on. Optional dependencies
count even when their features are not default; dev-only verification
assembly is not part of this shipped graph.

There is currently no automated check of the resolved, locked, all-features
dependency graph against this role policy — the check and its role-policy
file that used to enforce it are gone. This is an unproven invariant until a
