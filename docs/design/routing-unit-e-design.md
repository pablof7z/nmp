...
`Manual`/`Imported` constructors are `#[cfg(feature = "tooling")]` — a tooling
feature enabled by in-workspace diagnostics, **not** by `nmp-ffi` (the app SDK).
The *variants* always exist in the sealed enum; only
