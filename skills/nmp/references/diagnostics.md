...
`SourceStatus.awaitingAuth`/`authDenied` are declared states, populated from exact per-session AUTH bookkeeping only once a challenge has been received and reduced for `AccessContext::Nip42` demands; a connected protected session with no entry yet reads as `AwaitingAuth { AwaitingChallenge }`. On the write path they are reached in the ordinary course. On a protected read against a relay that challenges only in response to a request, they are not — see #1889.
