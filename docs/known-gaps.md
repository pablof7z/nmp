...
- **NIP-11 cache is process-local.** The engine now owns bounded one-shot
  ...
  HTTP under one three-second deadline; no waiter or worker admission capacity
  exists, and engine shutdown closes every waiter and drains the task even when
  app handles survive. Relay URL credentials are rejected before
  ...
  no raw body. With one shared response body per live distinct-relay flight, the
  exact hidden raw-body envelope is `(256 cached bodies + live flights) * 256
  KiB`; live-flight count follows caller-owned distinct-relay concurrency, not a
  hidden internal pool or admission limit. A 64-waiter matrix on one flight
  therefore carries shared snapshot pointers (a few KiB on the qualified
  64-bit target), not 64 additional bodies. This is deliberately a raw-body
  bound, not a total-RSS claim: parsed values, relay URLs, HTTP metadata, maps,
  allocator/container overhead, and task/channel scaffolding are additional
  costs outside that number. The
