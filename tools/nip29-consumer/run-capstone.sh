...
usage() {
    printf '%s\n' \
        ...
        '' \
        'Starts the unrestricted two-relay fixture, runs every Rust/NMP phase,' \
        'stages relay outages/restarts, tears the fixture down, and retains logs.'
}

die() {
    printf 'nmp-nip29-consumer runner: %s\n' "$*" >&2
    exit 1
}
...
cargo run --quiet -p nmp-consumer-nip29 -- online \
...
cargo run --quiet -p nmp-consumer-nip29 -- restart \
...
printf 'NMP NIP-29 Rust capstone passed; evidence: %s\n' "$evidence_dir"
