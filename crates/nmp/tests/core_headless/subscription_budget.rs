...
//!
//! Run narrated with:
//! `cargo test -p nmp --test core_headless subscription_budget -- --nocapture`
...
fn core_watching(count: u16) -> (EngineCore<MemoryStore>, LiveWire) {
    ...
    for index in 0..count {
        let effects =
            core.handle_and_flush(EngineMsg::Subscribe(limited_pinned_query(30_000 + index)));
        wire.apply(&effects);
    }
...
fn relay_row(core: &EngineCore<MemoryStore>) -> nmp::mechanism::core::RelayDiagnosticsSnapshot {
