...
- **Replaceable-on-insert via ONE mutating door** (`RedbStore::insert`; no public index/storage setter). Order: exact-id dedup FIRST (redelivery merges provenance, no index churn), THEN replaceable supersede. Winner = newest `created_at`, tie-break **lexicographically smallest id**. Kinds {0,3,10000-19999} by (pubkey,kind); {30000-39999} by (pubkey,kind,d). (doctrine D9/D4, aim.md, 08-eventstore.md)
- **Gap-free live handoff before Negentropy (NIP-77) — DEFAULT, not opt-in.** For a behaviorally proven relay, send a distinct live `REQ` with `limit:0`, wait for that exact EOSE, keep it open, and only then reconcile/backfill under separate NIP-77 and temporary-REQ ids. Reconnect repeats the same order. This closes the snapshot-to-live loss window: dedup can absorb overlap but cannot recover an event delivered by neither path. Small exact sets stay on plain REQ, and **unprobed capability = conservatively NO NIP-77, never assumed.** (#563; doctrine D2, 13-sync-engine.md)
...
**🔴 FOLLOW-UP CORRECTION (#858, 2026-07-27):** the "`nmp-nip29` composes the decoded entries into typed `GroupRef`/`RememberedGroups` values" clause above is itself now withdrawn — kept as evidence, per this file's reading rule. That composition applied no NIP-29 rule: it renamed `SimpleGroupEntry`'s fields, copied them, and silently dropped `malformed_item_count`, which made the NIP-29-facing result strictly LESS truthful than the NIP-51-owned decode while creating a second owner of one value (Noun Gate). It, its FFI/Swift/Kotlin mirrors, and the `nmp-nip29 -> nmp-nip51` dependency edge are all deleted. What is true now: `nmp-nip51` decodes the list once, as itself, and the app SELECTS one entry and passes its exact fields — a host `RelayUrl` and a `group_id` — into NIP-29's host-pinned constructors. Everything else in the correction above still holds.

**🔴 FOLLOW-UP CORRECTION (#1551, 2026-08-14):** the separate `nmp-nip51` component is now deleted. NIP-51 still defines kind:10009. The kind remains one tolerant, observational value with every evidence field preserved; its product/API placement moved into `nmp-nip29` because this repository uses it only as the remembered-host input to NIP-29, and the separate component split that single capability across Rust, FFI, Swift, Kotlin, features, and tests. No wrapper returned: the app still selects one exact entry and explicitly passes its host and group id into the NIP-29 relay scope. The parser still grants no signature, canonical-store, routing, or mutation authority.
