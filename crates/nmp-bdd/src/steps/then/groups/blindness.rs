...
#[then(regex = r#"^it finds no kind constant that NIP-29 does not itself define$"#)]
async fn gate_finds_no_alien_kind(w: &mut NmpWorld) {
    let (passed, said) = w.gate_outcome();
    assert!(
        passed,
        "the ownership gate does not pass on this tree: {said}"
    );
    let surface = w.group_surface();
    ...
    assert!(
        alien.is_empty(),
        "NIP-29 defines 9000-9022; this crate also binds {alien:?}"
    );
}

#[then(regex = r#"^it finds no fixed catalogue of content kinds$"#)]
async fn gate_finds_no_catalogue(w: &mut NmpWorld) {
    let (passed, said) = w.gate_outcome();
    assert!(
        passed,
        "the ownership gate does not pass on this tree: {said}"
...
#[then(regex = r#"^a publication path that branched on the kind fails the gate$"#)]
async fn gate_rejects_a_kind_branch(w: &mut NmpWorld) {
    let (passed, said) = w.gate_rejects_a_kind_branch();
    assert!(
        !passed,
        "a NIP-29 source that privileges kind 9 must FAIL the gate, but it passed: {said}"
    );
    let (still_passes, said_again) = w.gate_outcome_now();
    assert!(
        still_passes,
        "the probe must leave the tree exactly as it found it, but the gate now says: \
         {said_again}"
    );
}
