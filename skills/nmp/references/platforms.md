...
`Engine::sign_event(SignEventRequest)` freezes the active author and returns either a typed `SignEventStartError` or a cancellable `SignEventOperation`; once accepted, `recv` yields one fully verified event or a typed `SignEventFailure`. It never accepts or publishes a write. Asynchronous `SigningCapability` implementations create pending work with `SignerOp::pending_channel` or `pending_channel_with_cancel` and resolve the returned opaque `PendingSignerSender`; consumers do not receive or decompose NMP's internal channel.
