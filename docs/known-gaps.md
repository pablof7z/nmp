...
- **Fallible ingest/read store doors and internal handle reconstruction are built; native diagnostics remain deferred (#122/#1362).** The six ingest/read `EventStore` doors (`insert`/`query`/`remove`/`expire_due`/`record_coverage`/`gc`) return `Result<_, PersistenceError>` — `RedbStore` propagates real redb I/O errors instead of `.expect()`-panicking on every EVENT frame, and the engine latches a `DiagnosticsSnapshot.store_degraded` signal instead of crashing the host app. `EventStore::next_expiration` and `get_coverage` were subsequently widened by #763. A fault classified `requires_reopen()` now arms engine-owned, event-driven reconstruction with exponential backoff capped at 30 seconds: Redb drops only its failed database generation while retaining the canonical-target ownership fence, reopens and validates that exact store, and the same engine rebuilds its store-derived resolver and write state before declaring the store healthy. Deliberately still open: `store_degraded` is surfaced only on the Rust `DiagnosticsSnapshot`; it is NOT plumbed through `FfiDiagnosticsSnapshot`/Swift/Kotlin, so a native host cannot observe either the degraded interval or recovery. Invariant and value-too-large failures do not trigger handle reconstruction, and a permanently unavailable target remains degraded rather than fabricating success.
