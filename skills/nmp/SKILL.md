---
name: nmp
description: Build, review, debug, test, or plan applications and protocol modules that use the Nostr Multi-Platform (NMP) Rust public facade. Use for live queries, write intents and receipts, identity/signers, diagnostics, content parsing, the projected protocol doors (NIP-02, NIP-18, NIP-22, NIP-25, NIP-29 including NIP-51 simple-groups lists, NIP-65, NIP-C7), lifecycle and recovery, practical feature recipes, protocol extension, and consumer-facing API verification. Do not use this as authority for unverified internals or future VISION contracts.
---
...
Verified-Revision: `09ec1dccdbf313bb7152ea6db530c3f44a0129cc`
...
3. Identify the supported facade (`nmp`) plus any opt-in protocol crates the feature needs. Never substitute an internal crate for the supported facade.
...
- Rust setup, call maps, and test commands: [Platforms](references/platforms.md)
- Content parsing, or any projected protocol door (NIP-02, NIP-18, NIP-22, NIP-25, NIP-29 including NIP-51 simple-groups lists, NIP-65, NIP-C7): [Content and protocols](references/content-and-protocols.md)
- Adding or reviewing a protocol module or public API: [Protocol authoring](references/protocol-authoring.md)
...
- Keep query ownership explicit. A `Subscription` is eager once observed and withdraws on `Drop` or explicit `cancel()`.
...
- Do not promise app-controlled retries, typed pending-row metadata, or a platform-vault account store: those are not current public capabilities. `RelayWaiting::BackingOff` is evidence from NMP's engine-owned durable scheduler, not an app retry door. Write cancellation before signing, publish-queue enumeration and entry removal, `max_relays` config, and populated `AuthPhase` do exist in the current facade — check the current surface before calling something absent. No application-configurable worker/task/thread capacity exists: #704 removed it entirely.
...
3. Show deterministic query/signer/engine teardown and explicit fact-stream ownership. Detaching a receipt fact stream (dropping the `ReceiptStream`) is distinct from `Engine::cancel(id)`, which ends a not-yet-signed write obligation.
