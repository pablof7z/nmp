...
`LiveQuery` is the observation noun: `Engine::observe(query, window)`. A single demand is the one-branch case, built with `LiveQuery::single`.
...
`Derived.inner` is a full `Demand`. The inner
...
For backfill, prefer a windowed observation: pass a `Window::Expandable { initial, max }` to `observe` and grow it with `request_rows`, never above the window's `max`. `observe` refuses `WindowInitialExceedsMax`, `WindowSelectionHasLimit`, and `WindowAggregateResultLimit` rather than silently reinterpreting a conflicting request; a zero-row window is unrepresentable, since `Window::Expandable`'s `initial` and `max` are `NonZeroUsize`.

Growth is declarative and idempotent — `request_rows(at_least)` states the total you want, so there is no continuation token to go stale. Its only failures are `Unwindowed`, `EngineClosed`, and `StoreUnavailable`. The *outcome* arrives in band as the `WindowLoad` fact on the next frame (`Frame.window.load`): `Idle`, `Requesting`, `Returned { added }`, or `AtBound { max }`. `AtBound` is a fact, not an error, and `Returned { added: 0 }` means only that the planned advance added no canonical row — never that no older event exists. There is deliberately no Complete/End/Synced variant; read the per-source evidence for that judgment.

A `Subscription` withdraws itself on `Drop`; `cancel()` is the same thing spelled as intent. Because `recv()` blocks, a drain thread usually owns the subscription outright — `cancel_handle()` hands another thread an `ObservationCancel`, and `window_handle()` hands it a `WindowHandle` that can `request_rows` or `cancel` the same windowed observation. Every one of those paths converges on a single withdrawal, however many callers fire. An ordinary or windowed observation can fail with `EngineError::ObservationUnavailable` only when store degradation prevents its initial canonical projection from opening; relay connection failure remains source evidence.
