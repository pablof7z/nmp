...
A **Rust demo CLI** (`nmp-demo`) runs the engine against the *live* Nostr network: it discovered a real user's 193 follows + their NIP-65 relays and streamed ~1,200–2,600 real notes via outbox routing — the whole stack, end-to-end, no fixtures. Dogfooding it under real load also found a **P0 the 170 tests missed**: `subscribe` re-emits the full row set on every event (O(n²) redelivery) — fix in progress. Open gaps (this, the P0, and **reads-only account switching** — publishing as a switched account must be wired before M5's multi-nsec falsifier) tracked honestly in [`docs/known-gaps.md`](docs/known-gaps.md).
