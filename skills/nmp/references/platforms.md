...
Drop `--sim-only` when a physical-device slice is required. Rebuild the xcframework after a UniFFI change.
...
Swift `NMPConfig` has `storePath`, `appRelays`, `fallbackRelays`, `maxRelays` (default 10), and `maxAuthCapabilities` (default 64). It exposes no protocol-provider setting and no worker/task capacity field; the Rust fields it omits are `indexer_relays` and `max_publish_attempts`.
