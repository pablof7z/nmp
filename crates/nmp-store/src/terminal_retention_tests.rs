...
use crate::{
    sentinel_signature, AcceptOutcome, AcceptWrite, EventStore, IntentSigState, MemoryStore,
    ReceiptState, RedbStore, RefuseReason, RemoveQueueEntryOutcome,
};
...
fn accept(
    ...
    store
        .accept_write(AcceptWrite {
            frozen: frozen(keys, content, created_at),
...
fn exercise_global_fifo(store: &mut dyn RetentionTestStore) {
    ...
    assert_eq!(
        store.reattach_receipt(cancelled_id).unwrap().unwrap().state,
        ReceiptState::Cancelled
    );
    assert_eq!(
        store
            .reattach_receipt(unroutable_id)
            .unwrap()
            .unwrap()
            .state,
