    ...
    S: EventStore,
{
    ...
    loop {
        ...
        if !command_is_tick {
            // Every reducer transition sees current wall truth for facts it
            // may stamp. This is one O(1) assignment, never a maintenance
            // sweep; the due-deadline guard above remains Tick's sole owner.
