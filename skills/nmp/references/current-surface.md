...
Verified-Revision: `9a11815f6ef60ddc0982c37e8765e3bc49f505d7`
...
- NIP-11 snapshots are process-local and engine-owned; cross-process persistence is not shipped. The cache retains at most 256 last-good snapshots, including refreshing entries. Flights use the shared native executor with zero queue, each flight admits at most 64 waiters, and an uncached completion is delivered without retention when every cached entry is refreshing. The production request uses explicit Hickory DNS plus rustls HTTP under one three-second total deadline, follows no redirect, and performs no automatic retry. Native hostname tests cover Swift and Kotlin/JVM, but there is no Android/AAR qualification.
