...
async fn a_join_request_is_publishable_with_no_subscription_at_all() {
    ...
    let receipts = group
        ...
        .expect("a join request is accepted with no prior read").statuses;
...
async fn one_retained_group_handle_mints_every_read_and_write_with_no_lifecycle_of_its_own() {
    ...
    let receipts1 = group
        ...
        .expect("first write, from the retained handle").statuses;
    ...
    let receipts2 = group
        ...
        .expect("second write, from the retained handle, after both reads were dropped").statuses;
...
async fn a_moderation_rejection_reports_the_hosts_exact_message_and_is_never_accepted() {
    ...
    let receipts = group
        ...
        .expect("the door accepts the intent even though the host will refuse it").statuses;
...
async fn a_moderation_rejection_is_a_host_fact_not_a_routing_failure() {
    ...
    let receipts = group
        .remove_user(&engine, moderator, subject)
        .expect("the door accepts the intent for the whole scope").statuses;
