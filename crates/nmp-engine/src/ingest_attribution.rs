...
pub fn bridge_batch(frames: usize, event_bytes: usize) {
...
pub fn bridge_send(duration: Duration) {
    add(&BRIDGE_SEND_NS, duration);
}
pub fn bridge_applied_wait(duration: Duration) {
    add(&BRIDGE_APPLIED_WAIT_NS, duration);
}
pub fn engine_batch_process(duration: Duration) {
    add(&ENGINE_BATCH_PROCESS_NS, duration);
}

pub fn relay_core_reduce(duration: Duration) {
    add(&RELAY_CORE_REDUCE_NS, duration);
}

pub fn relay_core_reduce_cpu(nanos: u64) {
...
pub fn relay_effect_dispatch(duration: Duration) {
...
pub fn committed_observation_effect(duration: Duration) {
    add(&COMMITTED_OBSERVATION_EFFECT_NS, duration);
}

pub fn diagnostics_effect(duration: Duration) {
...
pub fn history_channel_send(duration: Duration) {
    add(&HISTORY_CHANNEL_SEND_NS, duration);
}

pub fn history_receiver_reconcile(duration: Duration) {
    add(&HISTORY_RECEIVER_RECONCILE_NS, duration);
}

pub fn row_channel_send(duration: Duration, deltas: usize) {
