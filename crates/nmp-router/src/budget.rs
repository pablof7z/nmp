...
mod tests {
    ...
    #[test]
    fn a_bare_cap_advertises_nothing() {
        let budget = CompileBudget::from(12);
        assert_eq!(budget.relay_cap(), 12);
        assert_eq!(budget.max_subscriptions(&relay()), None);
        assert_eq!(budget.max_subid_length(&relay()), None);
        assert!(!budget.rejects_our_subscription_ids(&relay()));
    }

    /// A relay that advertised a `limitation` object WITHOUT
    /// `max_subscriptions` is exactly as unbudgeted as one that published no
    /// document: the distinction the engine keeps is `Option`, and a present
    /// document with an absent field must not collapse into a number.
    #[test]
    fn a_document_without_the_field_is_still_unbudgeted() {
        let budget = CompileBudget::with_relay_cap(12).advertising(
            relay(),
        ...
        assert_eq!(budget.max_subscriptions(&relay()), None);
        assert_eq!(budget.max_subid_length(&relay()), Some(100));
    }

    #[test]
    fn our_own_id_length_is_the_rejection_boundary() {
        let with = |length| {
            CompileBudget::with_relay_cap(12).advertising(
                relay(),
                AdvertisedRelayLimits {
                    max_subscriptions: None,
                    max_subid_length: Some(length),
                },
            )
        };
        assert!(with(WIRE_SUB_ID_CHARS - 1).rejects_our_subscription_ids(&relay()));
        assert!(!with(WIRE_SUB_ID_CHARS).rejects_our_subscription_ids(&relay()));
        assert!(!with(WIRE_SUB_ID_CHARS + 1).rejects_our_subscription_ids(&relay()));
    }
}
