Feature: A write publishes as the active account unless it names someone else
  Every write has exactly one identity, and there are exactly two ways to
  arrive at it: the active account, or a key the app named. Naming nobody is
  not the absence of a choice -- it is the choice "whoever is active when
  ...
  and because publishing as one of them must not require making it the active
  ...
  way to know. Under an active-account default the pin matters MORE than it
  used to, not less: the default is precisely the case where the app never
  named a key, so acceptance is the only place "whoever is active" becomes
  somebody.
  ...
  Background:
    ...
    And the account with pubkey "81b637d8fcd2c6da6359e6963113a1170de795e4b725b84d1e0b4cfd9ec58ce9" is registered with a working signer
    And my podcast identity "f62a697de0475d83990780a93267ba3113dcc90a84047574aeb274837df600fd" is registered with a working signer
  ...
  Scenario: Switching accounts never retargets a write that was already accepted
    ...
    # signer is asked.
    Given "2bd806c97f0e00af1a1fc3328fa763a9269723c8db8fac4f93af71db186d6e90" is the active account
    And the podcast identity's signer is slow to answer
  ...
  Scenario: An active-account write is pinned to whoever was active when it was accepted
    ...
    Given "2bd806c97f0e00af1a1fc3328fa763a9269723c8db8fac4f93af71db186d6e90" is the active account
    And that account's signer is slow to answer
    When I compose an event of kind 1 saying "hello" and publish it naming no identity
    And the write reports accepted
    And I switch the active account to "81b637d8fcd2c6da6359e6963113a1170de795e4b725b84d1e0b4cfd9ec58ce9"
  ...
  Scenario: A restart does not re-resolve an accepted write against the new session
    ...
    # resolution against whichever account happens to be active on the way
    # back up.
    Given "2bd806c97f0e00af1a1fc3328fa763a9269723c8db8fac4f93af71db186d6e90" is the active account
    And that account's signer is offline
    When I compose an event of kind 1 saying "hello" and publish it naming no identity
    And the write reports accepted and the process stops immediately
    And I reconstruct the engine from the same durable store with "81b637d8fcd2c6da6359e6963113a1170de795e4b725b84d1e0b4cfd9ec58ce9" active
    Then the pending write still awaits "2bd806c97f0e00af1a1fc3328fa763a9269723c8db8fac4f93af71db186d6e90"
