...
- **M3 — the engine runs, persists, and stays correct offline.** A dedicated engine thread (blocking `recv`, no polling) drives the pure reducer against real WebSocket relays: a `Send+Clone` `Handle` exposing only the two nouns over plain channels, with mio/tungstenite sealed inside. Live-proven end-to-end: subscribe→rows, durable publish→per-relay acks (two relays resolving to *different* terminals, so "sent" is only knowable from the receipt stream), reconnect→subscription replayed with no gap, NIP-77 negentropy reconciliation against a real relay, and — the capstone — a **cold-start offline read that is authoritative**: a fresh engine reopens the persisted store with no relay reachable and returns cached rows as `CompleteUpTo` from the stored watermark (a never-synced query correctly reads `Unknown`). Verified: coverage attribution can't credit an atom the wire never proved (intersection-race + limit-poisoning + authoritative-empty all falsified headlessly); `ProbedRelay` gate, `NarrowOnly` (no widen op), receipt-streams (never a bool), and the pure-reducer/no-runtime-leak architecture all hold.

Reviews: [`docs/reviews/`](docs/reviews/). Open gaps tracked honestly in [`docs/known-gaps.md`](docs/known-gaps.md) — most important: **account switching is currently reads-only** (the signer is fixed; publishing as a switched account must be wired before M5, whose falsifier needs multi-nsec switching), and there's no live liveness-sweep driver yet.
