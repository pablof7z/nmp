...
- Treat NIP-11 as an explicit engine-owned one-shot, not a relay stream or an app-owned cache. Its flights share the same zero-queue native executor, each relay has a finite waiter set, and executor saturation, waiter saturation, OS-thread refusal, service closure, HTTP failure, size refusal, and invalid JSON are distinct acquisition facts. A stale-on-error snapshot preserves the last-good document and carries the refresh error separately. Relay advertisement is evidence from one cited document revision; it never becomes behavioral capability proof.
