Feature: The routes that were removed stay removed
  ...
  their claim yet: that these particular retired spellings are among the ones
  a durable row cannot decode, that one such row does not obstruct its
  neighbours, and the runtime half of the bootstrap publish. #1320 owns
  writing those three proofs.
  ...
  Scenario Outline: A durable write journalled under an old spelling is kept, not reinterpreted
    ...
    # worse than leaving it inert. The restart falsifier in
    # `crates/nmp/tests/durable_accepted_restart.rs` proves that an unreadable
    # routing is retained and inert, but it stages one arbitrary string, so
    # nothing yet establishes that these particular spellings are among the
    # unreadable ones -- #1320.
