...
| 7 | **Cache evidence presented as global truth** | Query snapshots expose rows plus per-planned-source acquisition facts. No public state can express globally `synced`, `complete`, `authoritativeEmpty`, or `syncHealth`. | **TARGET.** Current aggregate `Unknown`/`CompleteUpTo` and builder claims overstate what relay watermarks can prove. |
