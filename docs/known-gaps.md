...
- **Unbounded historical replay can peg the main thread (M5 dogfooding finding), bound across two halves (#17).** `apps/Falsifier` (the M5 SwiftUI app) reproducibly saturates a simulator's main thread at ~97-98% CPU for 1-2 minutes, twice: (1) whenever a query without a `limit` (e.g. the app's `FeedFilters.followsRelayLists()`, `kinds:[10002]`) is freshly `observe`d, and (2) whenever `observeDiagnostics()` is first iterated. `sample` on the running process shows sustained top-of-stack time in `nmp_store::redb_store::RedbStore::EventStore::query` plus `serde_json`/schnorr-signature JSON parsing, not idle waiting -- real, repeated work, not a hang (it does eventually finish and CPU returns to 0%).
  ...
  - **Nested-JSON canonical event rows CLOSED (#150); ingest batching remains.** `EVENTS` and `OUTBOX_DISPLACED` no longer store a serde wrapper containing an inner NIP-01 JSON string. Their v2 values are an endian-defined binary envelope with raw id/pubkey/signature/time/kind fields and length-delimited tags, content, provenance, and local-owner metadata. Query predicates borrow fixed fields and tag/content slices directly from the redb value guard; rejected candidates never construct `nostr::Event`, parse hex, or reconstruct secp types, while only returned rows materialize an owned event. Opening a legacy database converts both tables and removes the JSON rows in one crash-atomic write transaction before publishing the schema marker. Differential matching tests pin equivalence with `nostr::Filter::match_event`; governed supersession/kind:5/NIP-40/GC/outbox tests stay unchanged. On the same real corpus/workload, the 50-iteration release mean is 0.310 ms — 2.44x faster than #142's final 0.757 ms rerun and 16.6x faster than the original 5.150 ms baseline. This remains a store microbenchmark; device room-open verification and #151 remain open.
  ...
  - **Parallel verification + single-writer batch ingest CLOSED (#151); device verification pending.** Transport workers still feed one pool-global verified-id/signature cache, but the translator now drains bursts of up to 128 frames and runs first-seen schnorr verification concurrently on native targets (the same code has a sequential wasm fallback); known ids remain cheap signature comparisons. The runtime preserves frame order while coalescing queued frames into one resolver call. `EventStore::insert_batch` then runs the exact governed insert state machine in input order inside one redb write transaction and commits once, including event rows, every ordered/tag/expiry/address index, kind:5 effects, provenance adoption, and outbox satisfaction; any persistence error aborts the whole batch. The resolver reacts once to the combined insert/remove set and the engine recompiles/refreshes once per burst. Both backends share contract tests for input-order supersession/provenance equivalence, and the full store/resolver/transport/engine suites remain green. Replaying 1,114 real canonical events into a fresh store averaged 5,216.268 ms as 1,114 individual transactions versus 28.215 ms as one batch across five release runs (184.88x). This isolates store transaction cost; end-to-end relay/device replay remains to be measured.
