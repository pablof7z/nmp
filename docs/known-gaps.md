...
- **Unbounded historical replay can peg the main thread (M5 dogfooding finding), bound across two halves (#17).** `apps/Falsifier` (the M5 SwiftUI app) reproducibly saturates a simulator's main thread at ~97-98% CPU for 1-2 minutes, twice: (1) whenever a query without a `limit` (e.g. the app's `FeedFilters.followsRelayLists()`, `kinds:[10002]`) is freshly `observe`d, and (2) whenever `observeDiagnostics()` is first iterated. `sample` on the running process shows sustained top-of-stack time in `nmp_store::redb_store::RedbStore::EventStore::query` plus `serde_json`/schnorr-signature JSON parsing, not idle waiting -- real, repeated work, not a hang (it does eventually finish and CPU returns to 0%).
  ...
  - **Ordered one-best-index query planning CLOSED (#149); device verification pending.** The author and kind indexes are binary `(field, created_at, !event-id)` rows, joined by global-created-at and tag indexes with the same suffix. `query_newest` chooses one best index (author, the smallest tag value set, kind, then global time), reverse-scans newest-first, and applies every remaining filter to the borrowed binary event. #646 removed the redundant physical author+kind index; a combined filter chooses the smaller author or kind bucket and post-filters the other exact predicate. Single ranges stop directly at the requested visible limit; OR values are exact k-way merges with id deduplication and the canonical `created_at DESC, id ASC` tie-break. All index mutations remain inside the same crash-atomic transaction as events, coverage, and outbox state; schema v7 atomically drops the retired v6 table and rebuilds planner cardinalities before publishing its marker. Tests prove kind/global scans materialize exactly N rows, multi-tag OR order is exact, rejected candidates stay borrowed, and legacy reopen migrates without losing canonical rows. On the real 1,062-row corpus, 100-iteration release means were 0.373 ms for the busiest room, 0.299 ms for kind:9, and 0.317 ms for the global newest 200. The original room baseline was 5.150 ms; end-to-end device room-open remains to be re-measured.
...
- **`nmp-blossom` covers the BUD verbs and their FFI/Swift/Kotlin projection; durable upload, BUD-03 placement, and `get`/`media` remain open (#545 upload, #551 mirror/delete/list, #555 projection, #730 composition, epic #216).** The opt-in crate ships the BUD-11 kind:24242 authorization vocabulary (draft builders for upload/delete/list — BUD-04 mirror deliberately reuses the `upload` builder — plus validate + header encoding), the BUD-02 blob-descriptor parser, and an HTTP client with the engine's admission discipline covering sha256-self-verifying `PUT /upload`, `PUT /mirror` (409/502 kept distinct, same integrity gate), single-blob-bound `DELETE /<sha256>`, and strictly parsed, bounded `GET /list/<pubkey>` (cursor/limit pagination; the deprecated `since`/`until` are not modeled). #555 projects all of it through `nmp-ffi` and the hand-written SDKs. #730 preserves the successful upload/mirror proof as a constructor-free `FfiVerifiedUpload`/`VerifiedUpload` object; list rows remain freely copyable, unverified descriptors and cannot be promoted into that witness. Signing note: FFI apps sign a Blossom draft through the existing governed sign-only surface (`NmpEngine::sign_event`), which freezes the author from the ACTIVE ACCOUNT — there is no per-operation identity override on the sign-only path (the `FfiWriteIntent.identity_override` seam from #550 covers publish intents only), so an app whose Blossom author differs from the active account must sign the draft's `unsigned_event_json` with an external/native signer and validate via `FfiBlossomAuthorization::validate`. Still open under #216: the `get`/`media` endpoints (the `BlossomVerb` enum models `get` totally, but it has no draft builder), engine-integrated durable upload (#562), and BUD-03 kind:10063 server-list placement.

- **`nmp-nip68` owns the NIP-68 kind:20 picture-first event build + decode with imeta artifact provenance; richer-tag layers remain open (#558, #730, epic #216 T15-B-NIP68-IMETA).** The opt-in crate exclusively claims kind:20 and, engine-free and signing-free (the `nmp-nip29`/`nmp-blossom` discipline), mints a `PictureImage` artifact reference only from a content-addressed Blossom `BlobDescriptor`/`VerifiedUpload` (`url`/`m`/`x` carried by construction; a descriptor without a mime type cannot mint one), builds an immutable unsigned kind:20 draft (`build_picture`, refusing zero images), and tolerantly decodes a kind:20 event into typed picture facts with recorded diagnostics (`decode_picture`, surfacing a missing `x` as `sha256: None` + `ImetaMissingSha256` rather than trusting it). #730 projects the first-cut `title` + `imeta` images + `content-warning` + `t` hashtags to FFI/Swift/Kotlin as an opaque picture draft whose sign request and ordinary write intent are projections of the same held event body. One shared real-upload fixture pins the exact event id, byte-for-byte unsigned JSON, tags, content, durability, and AuthorOutbox route across direct Rust, FFI, Swift, and Kotlin. Still open: richer event-level tags (`location`/geohash, annotate-user, `L`/`l` labels).

- **`nmp-media` provides the STANDALONE staged composition seam (prepare → upload → compose) with separated failure domains; durable upload and BUD-03 server-list placement remain open (#559, #730, epic #216 T15-C-MEDIA-COMPOSITION).** The opt-in crate wires the app-facing pipeline `Sha256Hash → signed authorization → VerifiedUpload → kind:20 draft` into three witness-typed stages so a skipped/failed stage is unrepresentable: `prepare` (holds the exact bytes it hashed and authorized — uploading those held bytes makes an authorized-hash/uploaded-bytes mismatch structurally impossible), the async standalone `PreparedUpload::upload` (a used-once obligation yielding a verified `UploadedAsset`), and `compose_picture` (the final unsigned kind:20 the app hands to the EXISTING `publish()` → WriteIntent path). It OWNS no event kinds and exports no `claims()` — composition is not schema ownership (`routing-and-ownership.md` §3.2.1; enrolled in `nmp-audit` as `DeclaresNoClaims`), and it never signs (signing is upstream) or publishes (relay/publish is downstream). The three failure domains are three SEPARATE types (`PrepareError`, `MediaUploadError`, `MediaComposeError`) so an upload failure can never be pattern-matched or `?`-merged as a compose failure; `MediaUploadError::Blossom` preserves the whole separated Blossom `UploadError` taxonomy intact. #730 adds a narrow native projection of the verified-upload → picture-draft seam without collapsing those domains or exposing arbitrary descriptors, `imeta`, kinds, or routes. The upload half remains NOT crash-durable: engine-integrated durable upload — persisted intent / reattachable receipt / HTTP-publish effect / blob persistence — is the additive #562. BUD-03 kind:10063 server-list placement is also still deferred.
