...
- **NIP-11 cache is process-local.** The engine now owns bounded one-shot
  ...
  257th fresh result is delivered but not retained. Fetches run on the internal
  fixed-capacity blocking-adapter pool (not any app-visible native-task ceiling;
  #680) and use cancellable Hickory DNS plus HTTP under
  ...
  no raw body. Under the internal adapter pool's fixed capacity (#680), the exact
  hidden raw-body envelope is 256 cached bodies plus a bounded number of active
  response bodies: `(256 + flights) * 256 KiB`. A fully
  admitted flight/64-waiter matrix therefore carries shared snapshot
  pointers (a few KiB on the qualified 64-bit target), not that many additional
...
- **Transport/verifier OS-thread ownership CLOSED (#442, #446); native observation ownership REPLACED by pull-based async handles (#680).** Every engine owns exactly two persistent native verifier workers (one on wasm's sequential path), one transport translator, one relay-retirement reaper, and one reaper for the internal blocking-adapter pool. `max_relays` bounds demanded live relay workers plus an equal charged retirement allowance. **#680 removed the one-OS-thread-per-observation bridge and the app-visible native-task ceiling entirely:** row, window, diagnostics, follow, receipt, and follow-action streams are now waker-driven async pull handles (`ObservationHandle::next()`) over engine-owned bounded mailboxes, so NMP OS-thread count is independent of live-observation count. `max_native_tasks`/`maxNativeTasks`/`ExecutorSaturated`/the native-task census/idle-barrier are gone from Rust, FFI, Swift, and Kotlin. Receipt/follow-action live facts use a fixed 32-item FIFO: overflow retains the prefix, prunes the stalled sink, and reports typed lag; receipt reattachment traverses deterministic durable pages of at most 32 facts using an identity-stable continuation bounded by relay fan-out, then atomically joins live work after a caught-up check. The cursor records consumed per-lane fact identities rather than a numeric offset into mutable reconstructed history, so a durable fact added between pages is delivered exactly once even when it sorts before already-consumed facts from another relay. Every live receipt sink also has a private registration identity tied to the consumer FIFO's close/drop hook, so cancellation removes the exact sink without waiting for another status from a potentially permanently parked receipt. This bounds live delivery, **not** total persisted attempt history: durable receipt/attempt retention and GC remain open under #46, and replay currently reconstructs that retained canonical history before selecting each delivery page. The retained internal pool is a fixed-capacity, app-invisible owner for genuinely-blocking *transient* adapters only (NIP-11 flights, remote-signer/AUTH waiters, the follow-action worker); OS spawn refusal there is still typed `ThreadUnavailable`. Engine-associated NIP-46 sessions own their own executor (bounded by app-identity session count), never the shared pool, so opening a query can never be refused because a signer session is open. Falsifiers: a 1,000-handle thread-scaling proof (0 thread growth), a 64+-observation dense-composition proof (no capacity refusal, initial state delivered), cancellation/shutdown wake-to-`None`, normal Swift loop-exit teardown, concurrent-`next()` misuse, a paused consumer driven through 40 durable retry cycles that receives explicit lag then traverses every persisted fact in finite pages, a two-relay mutation-between-pages proof that every fact is delivered exactly once, and 128 alternating close/drop reattachments on a receipt permanently parked at `AwaitingCapability` with the retained live-sink count returning to zero after every cycle. `docs/design/async-observation-handles.md` records the replacement architecture; `native-task-executor.md` is retained as the superseded record.
