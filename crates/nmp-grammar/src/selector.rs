...
pub enum Selector {
    ...
    Ids,
    /// Project each value of a single-letter tag (parameterized, not
    /// per-tag variants).
    Tag(TagName),
...
mod tests {
    ...
    fn selector_variants_are_ord_and_hash() {
        ...
        set.insert(Selector::Tag(TagName::new('p').unwrap()));
    ...
    fn tag_selector_distinguishes_by_tag_name() {
        let p = Selector::Tag(TagName::new('p').unwrap());
        let d = Selector::Tag(TagName::new('d').unwrap());
        assert_ne!(p, d);
    }
