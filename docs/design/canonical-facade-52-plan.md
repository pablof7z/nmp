...
Today there is no single supported public API. Two callers assemble the
...
The one supported Rust public API is a new library crate **`nmp`** that both
...
1. **A hard dependency boundary, not a documentation promise.** #52 requires
   ...
   transitively. That is enforceable: a public-API inventory of `nmp` is the whole product.
...
list *is* the public API — a feature, not a cost.
...
- Injecting a pre-built store/directory (needed by `nmp-bdd`, which spawns the
  ...
  contract." `Handle` does not widen; the facade wraps it.
...
- **`convert.rs`** — keeps the *string→typed marshaling* that only exists because
  ...
  `StoreOpenFailed` project identically to today.
...
entry points point at real `ws://127.0.0.1:port` URLs, so no mock and no
...
fact on both entry points, with no `Accepted` and no EVENT/REQ reaching the relay.
...
**Order:** A0 → A → (B ∥ C) → D. **A is the chokepoint.** The
...
- **Q1 — Facade home (structural, load-bearing).** Recommendation: dedicated
  ...
  workspace member and defines the public API.
- **Q2 — Where the `Signed`-payload verify ultimately lives.** Facade `publish`
  (this plan) makes it entry-point-agnostic for all *app* entry points. Pushing it
...
- **Invariant check — no collision found.** The facade wraps `Handle`; it does not
  widen it. The "one facade" goal is consistent with the existing Swift/Kotlin
  hand-written ergonomic wrappers: those remain *projections* of the one Rust
  public API via UniFFI.
...
structure is sound, and the public-API constraint is the lightest enforceable
...
The whole point of #52 is that a single supported public API stops being a documentation
claim. Only a separate crate makes it a *dependency-graph* fact: an app's
`Cargo.toml` names `nmp` and nothing else, and the public API
of `nmp` **is** the entire product. Expanding
...
wording anticipates; docs mark everything below `nmp` unstable.
...
- **This changes #41's just-merged FFI behavior** (sync typed error → stream
  `Failed`). The superseded path is the convert.rs verify; updated falsifiers
  are #41's tests reshaped to assert the stream terminal.
...
- **"One invariant-preserving public API" — honored, with Q2 strengthening it.**
...
2. **FFI behavior change on a just-merged API** (#41 sync error → stream
   terminal) ripples into Swift/Kotlin falsifiers. Bounded.
3. **`cargo-public-api` nightly-rustdoc dependency** can break on toolchain
   bumps; pinning contains it, and the failure mode is a red CI job,
