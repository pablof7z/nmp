...
prepare core "$repo_root/native/examples/core.toml"
prepare normal "$repo_root/native/examples/normal-client.toml" --platform kotlin-jvm
prepare all "$repo_root/native/examples/all.toml"
prepare mix "$repo_root/native/examples/representative-mix.toml"
...
checksum=$(sed -n 's/^[[:space:]]*fun[[:space:]]\+\([A-Za-z0-9_]*checksum_[A-Za-z0-9_]*\).*/\1/p' "$binding" | head -n 1)
if [[ -z "$checksum" ]]; then
...
rm -rf "$cache"
"$repo_root/scripts/nmp-native" prepare \
    --manifest "$repo_root/native/examples/normal-client.toml" \
    --platform android \
    --platform kotlin-jvm \
