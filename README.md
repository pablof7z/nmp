...
- 🧪 NIP-42 AUTH — real and tested inside the NIP-46 signer connection. On **content** relays, two of three planned waves are now merged. Wave 1 keys relay identity/attribution/coverage/admission by **session, not URL** (`AccessContext { Public, Nip42(pubkey) }` + `RelaySessionKey`), passing an adversarial identity-isolation review clean ([#539](https://github.com/pablof7z/nmp/pull/539)). Wave 2 adds the **AUTH reducer + epoch state machine**: challenge epochs, a frozen `kind:22242` auth-event template (id commits to every field), AUTH-OK kept structurally disjoint from a durable write ACK, and authenticated write sessions reintroduced at the 9 sites Wave 1 had reverted to `Public` — passed an eight-invariant adversarial review that found and fixed one MED missed-wakeup (a `WaitingAuth` write lane not re-driven across disconnect/reconnect/boot) ([#541](https://github.com/pablof7z/nmp/pull/541)). Honest framing: still the *foundation*, not shippable AUTH — no app-facing policy/registry surface, no "log in to a relay" call yet; the per-session `auth_sessions` diagnostics this wave landed Rust-facade-only, with the FFI/Swift/Kotlin projection deliberately deferred to Wave 3 rather than shipping a partial cross-language surface. Wave 3 (runtime capability binding + the real-WebSocket AUTH capstone) is next, under [#8](https://github.com/pablof7z/nmp/issues/8)
...
- **Merged this session — two of three AUTH waves are in:** NIP-42 Wave 1, access-scoped session identity ([#539](https://github.com/pablof7z/nmp/pull/539)), keyed relay identity/attribution/coverage/admission by `(relay, access)` instead of URL, closing the structural cross-account-credit gap (bug-class ledger #18) *before* any AUTH negotiation exists — passed an adversarial identity-isolation review clean (all six invariants sound). Wave 2, the AUTH reducer + epoch state machine ([#541](https://github.com/pablof7z/nmp/pull/541), part of [#8](https://github.com/pablof7z/nmp/issues/8)), adds challenge epochs, a frozen `kind:22242` auth-event template, AUTH-OK kept structurally disjoint from a write ACK, and authenticated write sessions — an eight-invariant adversarial review found and fixed one MED missed-wakeup (a `WaitingAuth` write lane stranded across disconnect/reconnect/boot), then re-verified clean. A governed 30,000-line facade ceiling forced a clean scoping call: raised to 31,000 for the reviewed Rust-side `auth_sessions` diagnostics growth, while the FFI/Swift/Kotlin projection of that same surface is deliberately deferred to Wave 3 rather than shipping a partial cross-language read-out. Frame it honestly: still the foundation, not shippable AUTH — no app-facing API, and writes still ride the `Public` session until challenged. Wave 3 (runtime capability binding + the real-WebSocket AUTH capstone) is next. Only the NIP-46 bunker AUTH path works end-to-end today.
...
- **In progress:** NIP-42 content-relay AUTH — Wave 1 (access-scoped session identity) and Wave 2 (the AUTH reducer + epoch state machine — challenge/response, AUTH-OK vs write-ACK separation, authenticated write sessions) are merged; Wave 3 (runtime capability binding + the real-WebSocket AUTH capstone) is next, under [#8](https://github.com/pablof7z/nmp/issues/8)
