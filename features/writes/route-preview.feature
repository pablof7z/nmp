Feature: Asking what a write would compute to, before committing to anything
  ...
  @designed @ledger-3
  Scenario: A DM preview names what the publish then uses
    # The same property through an entirely different resolver. If any routing
    # strategy ever grew a private estimate of its own, this is where it shows.
    Given Bob's DM relay list names "wss://inbox.bob.example" as his DM relay
    And I am logged in as my own account
    When I ask what a direct message to Bob would route to
    And I publish that same direct message
    Then the message goes to exactly the destinations the preview named
  ...
  Scenario: An explicitly addressed preview names what the publish then uses
    ...
    # pinning: the trivial case must go through the same one derivation.
    Given I am logged in as my own account
  ...
  Scenario: A destination the preview could not name is not conjured at publish
    ...
    # succeeding somewhere the app was told was unreachable.
    ...
    When I publish that same direct message
    Then the message goes to no destination at all
  ...
  Scenario: A recipient with no relay list is unreachable, not unknown
    ...
    # answering: Bob has never published a DM relay list. That is knowledge,
  ...
  Scenario Outline: The app can decide whether to allow a send at all
    ...
    # evidence has to be enough to make the choice without guessing.
    Given <knowledge>
    And I am logged in as my own account
    When I ask what a direct message to Bob would route to
    ...
    Examples:
      | knowledge                                               | verdict |
      | Bob's DM relay list names "wss://inbox.bob.example"     | allowed |
      | nothing is known yet about Bob's DM relay list          | not yet |
      | the indexers have settled that Bob has no DM relay list | refused |
  ...
  Scenario: A preview that could not answer has set the answer in motion
    ...
    # Opening a DM compose screen previews in order to render the button, and
    # that preview is what causes the recipient's relay list to be fetched.
    Given nothing is known yet about Bob's DM relay list
    And I am logged in as my own account
    When I ask what a direct message to Bob would route to
    Then the preview reports a destination still unknown for Bob
    And Bob's DM relay list is now being sought
    When Bob's DM relay list arrives naming "wss://inbox.bob.example"
    And I ask again what a direct message to Bob would route to
  ...
  Scenario: Previewing only ever widens what is being sought
    ...
    And I am logged in as my own account
    When I ask what a direct message to Bob would route to
    Then Bob's DM relay list is now being sought
