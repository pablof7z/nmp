...
- **Read *which* governance check is red before diagnosing it.** Each gate reports its own name; a skipped check is a verdict that was never rendered, not a pass.
...
- **Decomposing an oversized file (#1496 and friends)?** Run `bash scripts/report-source-concentration.sh --file <path>` (or with no flags for every package/flagged file) before and during the work — it separates production, test, documentation, and generated lines so inline `#[cfg(test)]` modules don't inflate the number you're chasing. It runs on demand rather than as a CI check on every PR on purpose: numbers from your own branch, run right now, are fresher and more trustworthy than a historical CI log, and a non-blocking check nobody is required to open is just ceremony.
