...
final class ContentDocumentTests: XCTestCase {
    ...
    func testPlainTextKeepsOccurrenceAndPreservesBarePubkey() {
        ...
        guard case .pubkey(let pubkey) = occurrence.target else {
            return XCTFail("expected exact pubkey locator")
        }
