...
A coverage-satisfied `MaxAge` handle retains the compact opening source proof
and watermarks that justified suppressing wire work; later routing changes do
not rewrite or restart it. Its rows still react to canonical-store changes.
Open a new observation to evaluate current routing, while global diagnostics
continue to describe active wire work rather than a hypothetical replan. See
