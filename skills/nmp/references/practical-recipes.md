...
Do not display `100% synced`, infer zero from missing coverage, or promise native fields for counters that stop short of your platform: `store_degraded` and `sessions_refused_by_subscription_budget` are reachable only from direct Rust, and `sessions_rejected_over_cap` stops at the raw UniFFI layer. `SourceStatus.awaitingAuth`/`authDenied`, `AuthPhase` and `DiagnosticsSnapshot.authSessions` exist but are NOT populated today against a relay that challenges in response to a request, which is to say against strfry (#1889). A connected protected session gets a hardcoded row — `epoch_sequence: None`, `challenge_hash: None`, `phase: AwaitingChallenge`, `policy_bound: false`, `signer_bound: false` (`nmp-engine/src/core/mod.rs:2959-2972`) — because the only constructor of the real state is reached from a challenge NMP never receives on that path. `authDenied` is unreachable. A relay-debug sheet built on these renders constants until #1889 closes.
