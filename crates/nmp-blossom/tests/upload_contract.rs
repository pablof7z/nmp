...
/// reads the FULL request -- request line, headers, AND the Content-Length
/// body -- before it replies or closes, so the client never observes an
/// early close (macOS mio ECONNRESET flakes).
mod support {
