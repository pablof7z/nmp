...
- replaceable conflict: `RefuseReason::ReplaceableBaseChanged { expected, actual }` keeps both optional ids for whole-value `ReplaceableEdit`; `actual: None` is not permission to invent an empty value. Replayable capabilities instead own an explicit first-value and source policy. NIP-02 supplies a complete empty kind:3, retains the operation, and automatically reapplies it over later source truth, so its typed follow/unfollow action has no stale-base retry workflow. A whole-value semantic refusal still enters custody as one terminal retained receipt with no accepted intent or downstream work;
