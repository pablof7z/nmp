...
rustdoc-derived public item inventory of `nmp`) and
...
files. The component extractor is a standalone, own-lockfile tool outside
`nmp-ffi` and the product workspace; steady-state CI sources it from the PR
base. **Implemented by Unit E.**
