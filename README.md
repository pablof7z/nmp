...
- 🧪 NIP-42 AUTH — real and tested inside the NIP-46 signer connection. On **content** relays, Wave 1 of the AUTH epic is now merged: a closed `AccessContext { Public, Nip42(pubkey) }` + `RelaySessionKey` keys relay identity/attribution/coverage/admission by **session, not URL**, passing an adversarial identity-isolation review (all six invariants sound) and making cross-account credit structurally impossible ([#539](https://github.com/pablof7z/nmp/pull/539)). Honest framing: this is the *foundation*, not authentication — no AUTH reducer, no challenge/response, no app-facing AUTH API yet, and writes still ride the `Public` session by design. Wave 2 (the AUTH reducer + epoch state machine) is next, under [#8](https://github.com/pablof7z/nmp/issues/8)
...
- **Merged this session — the first brick of content-relay AUTH:** NIP-42 Wave 1, access-scoped session identity ([#539](https://github.com/pablof7z/nmp/pull/539), part of [#8](https://github.com/pablof7z/nmp/issues/8)). Relay identity/attribution/coverage/admission are now keyed by `(relay, access)` instead of URL, closing the structural cross-account-credit gap (bug-class ledger #18) *before* any AUTH negotiation exists — passed an adversarial identity-isolation review clean (all six invariants sound). Frame it honestly: this is the foundation, not shippable AUTH — no AUTH reducer, no challenge/response, no app-facing API, and writes still ride the `Public` session. Wave 2 (the AUTH reducer + epoch state machine) is next. Only the NIP-46 bunker AUTH path works end-to-end today.
...
- **In progress:** NIP-42 content-relay AUTH — Wave 1 (closed access-scoped session identity, `AccessContext`/`RelaySessionKey`) is merged; Wave 2 (the AUTH reducer + epoch state machine — challenge/response, AUTH-OK vs write-ACK separation) is next, under [#8](https://github.com/pablof7z/nmp/issues/8)
