...
- **A protected-path change needs a different agent's signoff and a mosaico note to Pablo** (`conventions/protected-path-signoff.md`). The protected paths and prefixes are the ones `scripts/check-surface-migration-authorization.py` names in `PRODUCTION_POLICY` — the governance program, the surface tooling, `.github/workflows/{ci,architecture-gates,surface-governance}.yml`, `tools/behavior-traceability/` — never a copy of that list kept elsewhere. Before merging one: get a signoff from an agent that did not write the diff and did not sit in the author's chain, and post `mosaico channel send --channel /nmp --tag Pablo`. The signoff must by hand re-run what the gate's authorization abort skipped (change-log validation; deterministic double-order regeneration and snapshot staleness) and record the commands on the PR. This control is procedural because `master` has no branch protection (#608), so `surface-governance`/`surface-regeneration` block nothing on their own; it is scoped to protected paths only and does not reopen #592's ruling that review is evidence, never merge authority.
