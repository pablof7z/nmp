...
- 🧪 Blossom (BUD-11) media core — a new `nmp-blossom` crate ships kind:24242-authorized, sha256-verified blob upload plus mirror/delete/list, each with its own bound authorization ([#216](https://github.com/pablof7z/nmp/issues/216) epic, closes [#545](https://github.com/pablof7z/nmp/issues/545)/[#551](https://github.com/pablof7z/nmp/issues/551), [#552](https://github.com/pablof7z/nmp/pull/552)/[#557](https://github.com/pablof7z/nmp/pull/557)). Still direct-Rust only — the FFI + Swift + Kotlin projection ([#555](https://github.com/pablof7z/nmp/issues/555)) is open as [#560](https://github.com/pablof7z/nmp/pull/560): Rust/parity/falsifier/surface gates pass, but the Swift build itself is currently **red** (a real `[UInt8]`→`Data` type mismatch in the generated wrapper, not a flake); **no app-facing Blossom API on any platform yet**
...
- **Merged — Blossom (#216) core + verbs:** `nmp-blossom` ships BUD-11 kind:24242 upload authorization, sha256-verified blob upload ([#552](https://github.com/pablof7z/nmp/pull/552), closes [#545](https://github.com/pablof7z/nmp/issues/545)), and mirror/delete/list with per-verb bound authorizations ([#557](https://github.com/pablof7z/nmp/pull/557), closes [#551](https://github.com/pablof7z/nmp/issues/551)). Still direct-Rust only: the FFI + Swift + Kotlin projection ([#555](https://github.com/pablof7z/nmp/issues/555)) is in review as [#560](https://github.com/pablof7z/nmp/pull/560) — Rust tests, cross-SDK parity, falsifier-honesty, and surface-governance all pass, but the Swift and Kotlin package builds are currently **red** (a genuine `[UInt8]`→`Data` type mismatch in `Blossom.swift`, not flaky infra). Not merged; no platform has an app-facing Blossom call yet.
...
- Land the **Blossom** FFI + Swift + Kotlin projection ([#555](https://github.com/pablof7z/nmp/issues/555)) — in review as [#560](https://github.com/pablof7z/nmp/pull/560), currently blocked on a real Swift build error; land NIP-68 imeta ([#558](https://github.com/pablof7z/nmp/issues/558)) and upload-then-publish composition ([#559](https://github.com/pablof7z/nmp/issues/559)) behind it
...
- **Shipped:** Blossom (BUD-11) upload-authorization core and mirror/delete/list landed as `nmp-blossom` ([#552](https://github.com/pablof7z/nmp/pull/552)/[#557](https://github.com/pablof7z/nmp/pull/557)), direct-Rust only — see Protocol modules above.
