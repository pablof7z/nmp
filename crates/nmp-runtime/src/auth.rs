...
mod tests {
    ...
    fn policy_is_bound_before_callback_and_completes_the_exact_instance() {
        ...
        let expected_pubkey = match request_token.epoch.session.authenticate_as {
            Some(pubkey) => pubkey,
            None => unreachable!(),
        };
    ...
    fn absent_policy_and_signer_fail_closed_with_the_exact_token() {
        ...
        dispatch(
            AuthEffect::RequestPolicy {
                token: policy_token.clone(),
                expected_pubkey: match policy_token.epoch.session.authenticate_as {
                    Some(pubkey) => pubkey,
                    None => unreachable!(),
                },
        ...
        let unsigned =
            nostr::EventBuilder::auth("challenge", sign_token.epoch.session.relay.clone()).build(
                match sign_token.epoch.session.authenticate_as {
                    Some(pubkey) => pubkey,
                    None => unreachable!(),
                },
            );
    ...
    fn exact_connected_auth_flush_reports_the_exact_token_accepted() {
        ...
        assert!(matches!(
            crate::translate_pool_event(completion),
