...
current account, else fail closed pre-acceptance. `Some(pk)` is
...
acceptance pins `pk` so that "a later current-account change cannot retarget it,
...
request is "just use the current account."
...
- **`Active` with no current account fails closed pre-acceptance.** Master:
  "publishing with the current-account identity requires a current account"
  (`write.rs:1617-1627`) — no
...
- **`Explicit(pk)` with no available signing provider for `pk` PARKS durably as
  ...
  is retained and the status emitted so the exact frozen identity can resume
  when its provider becomes available), and it stays correct under the redesign — parking is that
