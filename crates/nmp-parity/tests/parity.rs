...
/// #1234: the direct and FFI listing doors must answer identically over every
/// combination of the two facts the rule reads -- the union's contents, the
/// whole-scope availability, and whether the record was even asked for.
///
/// This is the oracle that keeps `Listing::of` the one implementation. The
/// FFI side holds a copied-out RECORD rather than a `GroupSnapshot`, so it
/// supplies the two facts itself; if it ever grew its own opinion about the
/// negative -- claiming absence while a relay is still acquiring, or for a
/// record nobody requested -- exactly one of the cases below diverges.
#[test]
fn direct_and_ffi_listings_agree_on_every_settlement_and_selection_case() {
    use nmp::nip29::{GroupAvailability, GroupSnapshot, ListedSubject, Listing};
    use nmp_ffi::nip29::{
        member_listing, FfiGroupAvailability, FfiGroupRecord, FfiGroupSnapshot, FfiListedSubject,
        FfiListing,
    };
    use nostr::RelayUrl;

    let subject = Keys::generate().public_key();
    let host = RelayUrl::parse("wss://host-1.example.com").expect("a well-formed host");
    ...
    let selections = [
        (Vec::new(), Vec::new()),
        (
            vec![nmp::nip29::GroupRecord::Metadata],
            vec![FfiGroupRecord::Metadata],
        ),
        (
            vec![nmp::nip29::GroupRecord::Members],
            vec![FfiGroupRecord::Members],
        ),
    ];
    let unions: [(Vec<ListedSubject>, Vec<FfiListedSubject>); 3] = [
        (Vec::new(), Vec::new()),
        (
            vec![ListedSubject {
                pubkey: subject,
            ...
            vec![FfiListedSubject {
                pubkey: subject.to_hex(),
                role: Some("moderator".to_string()),
                hosts: vec![host.to_string()],
            }],
        ),
        (
            // A populated list that omits the subject: "this list has
            // entries" must never be confused with "this list has THIS one".
            vec![ListedSubject {
    ...
    for (direct_availability, ffi_availability) in availabilities {
        for (direct_selection, ffi_selection) in &selections {
            for (direct_union, ffi_union) in &unions {
                let direct = GroupSnapshot {
                    id: "photographers".to_string(),
                    metadata: None,
                    admins: Vec::new(),
                    members: direct_union.clone(),
                    availability: direct_availability,
                    per_host: BTreeMap::new(),
                    disagreements: BTreeSet::new(),
                    selected: direct_selection.iter().copied().collect(),
                }
                .member_listing(subject);

                let ffi = member_listing(
                    FfiGroupSnapshot {
                        id: "photographers".to_string(),
                        metadata: None,
                        admins: Vec::new(),
                        members: ffi_union.clone(),
                        availability: ffi_availability,
                        per_host: Vec::new(),
                        disagreements: Vec::new(),
                        selected: ffi_selection.clone(),
                    },
                    subject.to_hex(),
                )
                .expect("a well-formed subject");

                let projected = match &direct {
                    Listing::Named(entries) => FfiListing::Named {
                        entries: entries
                            .iter()
                            .map(|entry| FfiListedSubject {
                                pubkey: entry.pubkey.to_hex(),
                                role: entry.role.clone(),
                                hosts: entry.hosts.iter().map(RelayUrl::to_string).collect(),
                            })
                            .collect(),
                    },
                    Listing::Absent => FfiListing::Absent,
                    Listing::Unestablished => FfiListing::Unestablished,
                };
                assert_eq!(
                    ffi,
                    projected,
                    "direct and FFI disagreed for availability={direct_availability:?} \
                     selection={direct_selection:?} union_len={}",
                    direct_union.len()
                );
                cases += 1;
            }
    ...
    assert_eq!(
        cases, 36,
        "every availability x selection x union combination must be exercised"
    );
}
