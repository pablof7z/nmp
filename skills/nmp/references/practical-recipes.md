...
Do not display `100% synced`, infer zero from missing coverage, or promise native fields for counters that stop short of your platform: `sessions_refused_by_subscription_budget` is reachable only from direct Rust, and `sessions_rejected_over_cap` stops at the raw UniFFI layer. Do not plan a local-store health indicator at all: no tier reports one, because a durable-store failure has no observable state — the operation fails, the engine carries on, and what was lost was progress, not an accepted write. `SourceStatus.awaitingAuth`/`authDenied`, `AuthPhase` and `DiagnosticsSnapshot.authSessions` are populated on the WRITE path and NOT on a protected READ. A write always reaches AUTH — the connect-time probe finds a challenge, or the write provokes one — and constructs real per-session state — policy consulted, signer bound, `authDenied` reachable. A protected read withholds its request until AUTH completes, so on a relay that challenges only in response to a request it never provokes the challenge that would construct that state; its row stays the hardcoded placeholder — `epoch_sequence: None`, `challenge_hash: None`, `phase: AwaitingChallenge`, `policy_bound: false`, `signer_bound: false` (`nmp-engine/src/core/mod.rs:2955-2973`). A relay-debug sheet renders real rows for writes and constants for protected reads until #1889 closes.
