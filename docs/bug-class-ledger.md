...
| 16 | **Durable obligation lost or duplicated by retry layers** | Outbox is the sole persisted `(intent, relay)` attempt owner; transport owns sockets only; signer adapter owns one RPC; one deadline scheduler owns time. | **PARTIAL.** Versioned exact-byte ordinal/outcome ownership and Durable/AtMostOnce restart behavior are built; eligibility/backoff/caps/deadline scheduling remain #79. |
