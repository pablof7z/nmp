...
- ✅ BUD-03 server discovery — `nmp-blossom` exclusively owns kind:10063 and exposes the active account's server list as an ordinary reactive `Demand` (`AuthorOutboxes + Public`), with ordered closed decoding and malformed-tag evidence through Rust, FFI, Swift, and Kotlin ([#731](https://github.com/pablof7z/nmp/issues/731)). Signed-list and operator-configured endpoints combine only under an explicit provenance policy; every candidate crosses the existing URL plus post-DNS SSRF gate, and only an operator allowlist—not a signature—can admit local/private addresses.
...
- **Headline (merged) — upload-then-publish composition (T15-C) is in; Blossom epic #216's core arc is complete.** [#575](https://github.com/pablof7z/nmp/pull/575) closes [#559](https://github.com/pablof7z/nmp/issues/559): a new `nmp-media` crate wires `prepare → upload → compose` as three witness-typed stages over the *existing* `publish()` path, with upload failure, publish failure, and success kept as three genuinely separate error types rather than one collapsed boolean. Combined with T15-A (Blossom core/verbs/SDK, [#552](https://github.com/pablof7z/nmp/pull/552)/[#557](https://github.com/pablof7z/nmp/pull/557)/[#560](https://github.com/pablof7z/nmp/pull/560)) and T15-B (NIP-68 picture schema, [#566](https://github.com/pablof7z/nmp/pull/566)/[#568](https://github.com/pablof7z/nmp/pull/568)), the full upload→picture-event→publish arc is proven end-to-end in Rust. BUD-03 discovery is now typed across Rust and both native SDKs ([#731](https://github.com/pablof7z/nmp/issues/731)); epic [#216](https://github.com/pablof7z/nmp/issues/216) stays open, honestly, for the batched FFI/Swift/Kotlin projection of NIP-68 + `nmp-media` (Rust-only today) and durable upload as an additive upgrade ([#562](https://github.com/pablof7z/nmp/issues/562)).
