...
//!
//! Usage:
//! `cargo run -p nmp-store --release --example scale_bench -- <store.redb> [canonical_rows] [iterations]`
...
const BASELINE_NMP_COMMIT: &str = "126f0ac1e06c60724ea269a0eca74df5a5595039";
const STORE_FORMAT: &str = "pre-nostrdb-json";

#[derive(Default)]
struct BuildStats {
    insert_attempts: u64,
    buffer_flushes: u64,
    insert_transactions: u64,
    generation: Duration,
    insertion: Duration,
}
...
impl FixtureManifest {
    fn assert_config(&self, canonical_rows: u64) {
        ...
        assert_eq!(self.nmp_commit, BASELINE_NMP_COMMIT);
        assert_eq!(self.store_format, STORE_FORMAT);
...
fn flush_batch(
    ...
    stats.insert_attempts += events.len() as u64;
    let started = Instant::now();
    for (event, observed) in events {
        store.insert(event, observed).expect("insert scale event");
        stats.insert_transactions += 1;
    }
...
fn legacy_query_newest(
    store: &RedbStore,
    filter: &Filter,
    limit: usize,
) -> Vec<nmp_store::StoredEvent> {
    let mut rows = store.query(filter).expect("complete legacy query");
    rows.sort_unstable_by(|lhs, rhs| {
        rhs.event
            .created_at
            .cmp(&lhs.event.created_at)
            .then_with(|| lhs.event.id.cmp(&rhs.event.id))
    });
    rows.truncate(limit);
    rows
}
...
fn bounded_case(
    ...
    let warm = legacy_query_newest(store, filter, limit);
    ...
    for _ in 0..iterations {
        let started = Instant::now();
        let rows = legacy_query_newest(store, filter, limit);
...
fn main() {
    ...
    println!("nmp_commit={BASELINE_NMP_COMMIT}");
    println!("store_format={STORE_FORMAT}");
    println!("bounded_projection=complete_query_then_sort_truncate");
    ...
    if let Some(stats) = build {
        ...
        println!("buffer_flushes={}", stats.buffer_flushes);
        println!("insert_transactions={}", stats.insert_transactions);
        println!(
            "generation_ms={:.3}",
    ...
    if let Some(manifest) = existing_manifest {
        ...
        let manifest = FixtureManifest {
            ...
            nmp_commit: BASELINE_NMP_COMMIT.to_owned(),
            store_format: STORE_FORMAT.to_owned(),
            canonical_rows,
