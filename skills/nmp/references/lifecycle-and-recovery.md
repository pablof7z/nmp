...
Native tracked/composed publish starts the receipt bridge as async work on the shared runtime before calling core acceptance, and composed publish does so before taking its intent. There is no capacity or thread refusal on this path, so a returned receipt handle reflects an accepted obligation and consumed intent. After a successful return, persist the id promptly: process loss before app persistence remains unrecoverable because receipt enumeration is absent.
