...
`from_parts` is hidden behind `unstable-mechanism` for in-repo tests and is not an application assembly path. Test any touched Rust crate with `cargo test -p <crate>`; `cargo test --workspace` is the merge gate.
