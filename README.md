...
| M4 — Swift SDK boundary | planning (Opus) |
| M5–M6 | not started |

**Proved so far (running code, each independently verified by a separate Opus review):**
- **M1 — the reactive filter-binding grammar is general.** A headless resolver proves surgical re-routing via *one shared code path* at two depths (`$myFollows` = exactly close-C/open-D, zero churn on unchanged authors; depth-2 NIP-29 groups; account-switch re-root with no cross-account leak; `follows − mutes` resolved in-engine). Verified honest: zero kind-branches, real pipeline (v1's C5 synthetic-stand-in mode retired), no silent rebuild, a third shape needing zero engine change.
- **M2 — routing correct, coalescing off the correctness path.** Per-relay compilation with a 2-relay-min coverage solver + fan-out cap and widen-only coalescing (mandatory local re-filter), so a wrong merge rule costs bandwidth, not correctness — proven by property tests + a differential oracle. The kill measurement did not fire (600→15 wire subs). CI gates every push.
- **M3 — the engine runs, persists, and stays correct offline.** A dedicated thread (blocking `recv`, no polling) drives the pure reducer against real relays behind a `Send+Clone` handle (mio/tungstenite sealed inside). Live-proven: subscribe→rows, durable publish→per-relay acks (two relays → *different* terminals, so "sent" is only knowable from the stream), reconnect→replay with no gap, NIP-77 negentropy, and the capstone — a **cold-start offline read that is authoritative** (fresh engine, persisted store, no relay: cached rows as `CompleteUpTo`; a never-synced query reads `Unknown`). Coverage can't credit an atom the wire never proved.

A **Rust demo CLI** (`nmp-demo`) runs the engine against the *live* Nostr network: it discovered a real user's 193 follows + their NIP-65 relays and streamed ~1,200–2,600 real notes via outbox routing — the whole stack, end-to-end, no fixtures. Dogfooding it under real load found — and we then **fixed** — a P0 the 170 tests missed: `subscribe` re-emitted the full row set on every event (O(n²)); row delivery is now incremental (`Added`/`Removed` deltas), confirmed on the live network at a 1.0× ratio (was ~1294×). Open gaps — **reads-only account switching** (publishing as a switched account must be wired before M5's multi-nsec falsifier) and a non-reactive `RelayDirectory` — tracked honestly in [`docs/known-gaps.md`](docs/known-gaps.md).

**Not yet proved:** the SDK boundary (M4); the falsifier app (M5 — the second and final thesis-gate: does a normal iOS dev experience this as a library, not a framework?); Android (M6). **Disproved so far:** nothing.
