...
- 🧪 NIP-42 AUTH — real and tested inside the NIP-46 signer connection. On **content** relays, four of the planned waves are now merged. Wave 1 keys relay identity/attribution/coverage/admission by **session, not URL** (`AccessContext { Public, Nip42(pubkey) }` + `RelaySessionKey`), passing an adversarial identity-isolation review clean ([#539](https://github.com/pablof7z/nmp/pull/539)). Wave 2 adds the **AUTH reducer + epoch state machine**: challenge epochs, a frozen `kind:22242` auth-event template (id commits to every field), AUTH-OK kept structurally disjoint from a durable write ACK, and authenticated write sessions reintroduced at the 9 sites Wave 1 had reverted to `Public` — passed an eight-invariant adversarial review ([#541](https://github.com/pablof7z/nmp/pull/541)). Wave 3 adds **runtime capability binding** (`AuthPolicy` trait, bounded registry, `Handle::{add,remove}_auth_policy`) and a **real-WebSocket AUTH capstone**: an in-repo strict relay proves `challenge → policy → sign → AUTH → OK → REQ → EOSE → rows` end-to-end, plus denial-parking, a fresh challenge on reconnect, and a wrong-challenge oracle — all 8 lifecycle/leak invariants passed adversarial review clean ([#542](https://github.com/pablof7z/nmp/pull/542)). Wave 5 projects that onto the **app-facing Rust facade**: a registrable `AuthPolicy` trait, `add_account -> AccountRegistration` / `remove_account(&AccountRegistration)` (closes [#495](https://github.com/pablof7z/nmp/issues/495)), and per-session auth diagnostics — every type **facade-owned** rather than re-exported, so the governed 30,000-line facade ceiling actually **shrank** (29,957 → 29,557) instead of being raised ([#543](https://github.com/pablof7z/nmp/pull/543)). Honest framing: the engine boundary and the Rust app API are both proven now; there's still no "log in to a relay" call from Swift/Kotlin. Wave 6 (FFI + Swift + Kotlin projection) is in flight, under [#8](https://github.com/pablof7z/nmp/issues/8)
...
- **Headline (merged) — NIP-42 Wave 5: the app-facing Rust API is in.** Wave 1, access-scoped session identity ([#539](https://github.com/pablof7z/nmp/pull/539)), keyed relay identity/attribution/coverage/admission by `(relay, access)` instead of URL, closing the structural cross-account-credit gap (bug-class ledger #18) *before* any AUTH negotiation exists — passed an adversarial identity-isolation review clean. Wave 2, the AUTH reducer + epoch state machine ([#541](https://github.com/pablof7z/nmp/pull/541)), adds challenge epochs, a frozen `kind:22242` auth-event template, AUTH-OK kept structurally disjoint from a write ACK, and authenticated write sessions — an eight-invariant adversarial review found and fixed one MED missed-wakeup, then re-verified clean. Wave 3, runtime capability binding + the real-WebSocket AUTH capstone ([#542](https://github.com/pablof7z/nmp/pull/542), part of [#8](https://github.com/pablof7z/nmp/issues/8)), proves it against a real challenging relay: `challenge → policy → sign → AUTH → OK → REQ → EOSE → rows`, denial-parking, a fresh challenge on reconnect, and a wrong-challenge oracle — all 8 lifecycle/leak invariants passed adversarial review. Wave 5 ([#543](https://github.com/pablof7z/nmp/pull/543)) now projects that onto the supported `nmp` facade: a registrable `AuthPolicy` trait, `add_account -> AccountRegistration` / `remove_account(&AccountRegistration)` (closes [#495](https://github.com/pablof7z/nmp/issues/495)), and per-session auth diagnostics — landed under the governed facade ceiling via facade-owned types (following the `bc8fb97` precedent), so the snapshot actually **shrank** 29,957 → 29,557/30,000 rather than raising the guardrail. An independent adversarial review passed clean on mirror-conversion completeness, exact-instance registration semantics, and strict delegation of the pending/cancel machinery. Frame it honestly: the engine boundary *and* the Rust app API are both proven now; Wave 6 (FFI + Swift + Kotlin projection) is in flight — until it lands there's still no "log in to a relay" call from Swift/Kotlin, so only the NIP-46 bunker AUTH path is shippable end-to-end today.
...
- **In progress:** NIP-42 content-relay AUTH — Wave 1 (access-scoped session identity), Wave 2 (the AUTH reducer + epoch state machine), Wave 3 (runtime capability binding + the real-WebSocket AUTH capstone) and Wave 5 (app-facing Rust facade `AuthPolicy` API + `AccountRegistration`) are all merged; Wave 6, the final wave — FFI + Swift + Kotlin projection — is in flight now, under [#8](https://github.com/pablof7z/nmp/issues/8)
