...
- **`nmp-blossom` covers BUD verbs plus typed BUD-03 discovery across Rust/FFI/Swift/Kotlin, but not the durable upload or remaining composition projections (#545/#551/#555/#731, epic #216).** The opt-in crate ships the BUD-11 kind:24242 authorization vocabulary (draft builders for upload/delete/list — BUD-04 mirror deliberately reuses the `upload` builder — plus validate + header encoding), the BUD-02 blob-descriptor parser, and an HTTP client with the engine's admission discipline covering sha256-self-verifying `PUT /upload`, `PUT /mirror` (409/502 kept distinct, same integrity gate), single-blob-bound `DELETE /<sha256>`, and strictly parsed, bounded `GET /list/<pubkey>` (cursor/limit pagination; the deprecated `since`/`until` are not modeled). #555 projects all of it through `nmp-ffi` and the hand-written SDKs. #731 adds exclusive kind:10063 ownership, an ordinary active-account `Demand` (`AuthorOutboxes + Public`), ordered closed decoding with malformed/content evidence, and explicit `SignedListOnly | OperatorOnly | SignedListThenOperator` candidate qualification. A signed URL is never network authority: syntax, literal-host, and post-DNS SSRF admission run before selection and again at operation time; only `BlossomClientConfig.allowed_local_hosts` can admit a local endpoint, and every returned decision retains signed-list versus operator provenance. Replacement, deletion, expiry, absence, acquisition evidence, and account rerooting remain the canonical store/query lifecycle—there is no Blossom cache or observation verb. Signing note: FFI apps sign a Blossom draft through the existing governed sign-only surface (`NmpEngine::sign_event`), which freezes the author from the ACTIVE ACCOUNT; a differing author must use an external/native signer and validate via `FfiBlossomAuthorization::validate`. Still open under #216: durable upload (#562), richer `get`/`media` coverage, and the native projection gaps recorded in the following NIP-68/media rows.
