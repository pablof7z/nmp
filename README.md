...
- 🧪 Upload-then-publish composition — a standalone seam that ties a Blossom upload to an NIP-68 publish while keeping **upload failure, publish failure, and success as three separated outcomes by construction**, never one collapsed boolean ([#559](https://github.com/pablof7z/nmp/issues/559), T15-C). In CI as [#575](https://github.com/pablof7z/nmp/pull/575) — not yet merged.
...
- **In CI, not merged — upload-then-publish composition (T15-C):** [#575](https://github.com/pablof7z/nmp/pull/575) implements [#559](https://github.com/pablof7z/nmp/issues/559): a standalone seam over upload + publish that keeps upload failure, publish failure, and success as three separated outcomes by construction. Cross-SDK parity and falsifier-honesty gates are passing as of this writing; Rust tests, Swift package, Kotlin package, and surface-regeneration were still running.
...
- Land upload-then-publish composition ([#559](https://github.com/pablof7z/nmp/issues/559), T15-C — in CI as [#575](https://github.com/pablof7z/nmp/pull/575)) on top of the now-merged Blossom SDK projection and NIP-68 picture-event support
- Project NIP-68 through FFI/Swift/Kotlin (currently Rust-only)
- Revisit engine-integrated durable upload as an additive upgrade over standalone async upload ([#562](https://github.com/pablof7z/nmp/issues/562)), once T15-C composition lands
...
- `crates/nmp-{nip02,nip29,nip51,content,blossom,nip68}` — opt-in protocol modules
