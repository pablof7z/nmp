...
- 🧪 NIP-42 AUTH — real and tested inside the NIP-46 signer connection. On **content** relays, three of the planned waves are now merged and **authenticated reads work end-to-end at the engine level**. Wave 1 keys relay identity/attribution/coverage/admission by **session, not URL** (`AccessContext { Public, Nip42(pubkey) }` + `RelaySessionKey`), passing an adversarial identity-isolation review clean ([#539](https://github.com/pablof7z/nmp/pull/539)). Wave 2 adds the **AUTH reducer + epoch state machine**: challenge epochs, a frozen `kind:22242` auth-event template (id commits to every field), AUTH-OK kept structurally disjoint from a durable write ACK, and authenticated write sessions reintroduced at the 9 sites Wave 1 had reverted to `Public` — passed an eight-invariant adversarial review ([#541](https://github.com/pablof7z/nmp/pull/541)). Wave 3 adds **runtime capability binding** (`AuthPolicy` trait, bounded registry, `Handle::{add,remove}_auth_policy`) and a **real-WebSocket AUTH capstone**: an in-repo strict relay proves `challenge → policy → sign → AUTH → OK → REQ → EOSE → rows` end-to-end, plus denial-parking, a fresh challenge on reconnect, and a wrong-challenge oracle — all 8 lifecycle/leak invariants passed adversarial review clean ([#542](https://github.com/pablof7z/nmp/pull/542)). Honest framing: this proves the **engine boundary**, not shippable app AUTH yet — there's still no app-facing `AuthPolicy`/registry API and no "log in to a relay" call from Swift/Kotlin. Wave 5 (the Rust-facade surface for that API, plus reconciling `add_account`/`remove_account` into the ratified `AccountRegistration` shape) is in the build gate now; Wave 6 (FFI + Swift + Kotlin projection) follows, under [#8](https://github.com/pablof7z/nmp/issues/8)
...
- **Headline (merged) — NIP-42's hardest half is done and proven:** an authenticated content-relay read now works end-to-end **at the engine level**. Wave 1, access-scoped session identity ([#539](https://github.com/pablof7z/nmp/pull/539)), keyed relay identity/attribution/coverage/admission by `(relay, access)` instead of URL, closing the structural cross-account-credit gap (bug-class ledger #18) *before* any AUTH negotiation exists — passed an adversarial identity-isolation review clean. Wave 2, the AUTH reducer + epoch state machine ([#541](https://github.com/pablof7z/nmp/pull/541)), adds challenge epochs, a frozen `kind:22242` auth-event template, AUTH-OK kept structurally disjoint from a write ACK, and authenticated write sessions — an eight-invariant adversarial review found and fixed one MED missed-wakeup, then re-verified clean. Wave 3, runtime capability binding + the real-WebSocket AUTH capstone ([#542](https://github.com/pablof7z/nmp/pull/542), part of [#8](https://github.com/pablof7z/nmp/issues/8)), proves it against a real challenging relay: `challenge → policy → sign → AUTH → OK → REQ → EOSE → rows`, denial-parking, a fresh challenge on reconnect, and a wrong-challenge oracle — all 8 lifecycle/leak invariants passed adversarial review. Frame it honestly: the *engine boundary* is proven; the app-facing SDK surface (`AuthPolicy` API + account registration through Swift/Kotlin) is not merged yet. Wave 5 (the Rust facade for that surface) is in the build gate now; Wave 6 (FFI + Swift + Kotlin) follows. Only the NIP-46 bunker AUTH path is shippable end-to-end today.
- **Superseded:** [`remove_account` (#529)](https://github.com/pablof7z/nmp/pull/529) was closed — its pubkey-only shape contradicted #8's ratified `AccountRegistration` model. It's being replaced by Wave 5's `add_account -> AccountRegistration` / `remove_account(&AccountRegistration)`, which also closes [#495](https://github.com/pablof7z/nmp/issues/495).
...
- **In progress:** NIP-42 content-relay AUTH — Wave 1 (access-scoped session identity), Wave 2 (the AUTH reducer + epoch state machine) and Wave 3 (runtime capability binding + the real-WebSocket AUTH capstone) are merged, proving authenticated reads end-to-end at the engine level; Wave 5 (app-facing facade/FFI `AuthPolicy` API + `AccountRegistration`) is in the build gate next, then Wave 6 (Swift/Kotlin projection), under [#8](https://github.com/pablof7z/nmp/issues/8)
